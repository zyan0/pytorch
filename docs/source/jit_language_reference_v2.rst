...
::
    ...
    `integer <https://docs.python.org/3/reference/lexical_analysis.html#integer-literals>`
...
Lists and dicts can be constructed by either either listing the container contents explicitly or providing
...
`primary` must desugar (rather than evaluate) to a callable object. All argument expressions are evaluated
...
operands can be ``int``, ``float`` or ``Tensor``. Scalars are broadcasted in the case of scalar-tensor/tensor-scalar
...
have the same shape. For scalar-tensor or tensor-scalar ops, the scalar is usually broadcasted to the size of the
...
shape. When one is a scalar and the other is a ``Tensor``, the scalar is logically broadcasted to match the size of
the ``Tensor``.
...
the left operand is ``Tensor`` , the right operand is logically broadcasted to match the shape of the ``Tensor``.
...
""""""""""""""""""""
For any types stored inside ``IValue`` as pointers, operators ``is`` and ``is not`` test for the object’s identity;
